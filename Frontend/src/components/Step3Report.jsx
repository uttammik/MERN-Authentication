...
const Step3Report = ({report}) => {
  ...
  const downloadPDF = ()=>{
    ...
    if(finalScore>=8){
      ...
      advice = "Good foundation shown. Improve clarity and structure. Practise delivering concise, confident answers with stronger supporting examples."
    }else{
      advice = "Significant improvement required. Focus on structured thinking, clarity, and confident delivery. Practise answering aloud regularly."
    }
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-gray-50 to-green-50 px-2 sm:px-6 lg:px-10 py-4'>
      <div className='mb-1 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
          <div className='md:mb-4 w-full flex items-start gap-2 flex-wrap'>
            <button 
          ...
          <button 
            onClick={downloadPDF}
            className='bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 text-nowrap rounded-xl shadow-md transition-all duration-300 font-semibold text-sm sm:text-base'>
            Download PDF
          </button>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8'>
        <div className='space-y-3'>
          <motion.div
            initial={{opacity:0}}
            animate={{opacity:1}}
            className='bg-white rounded-2xl sm:rounded-3xl shadow-lg p-3 sm:p-8 text-center '
            >
            <h3 className='text-gray-500 mb-4 sm:mb-6 text-sm sm-text-base'>Overall Performance</h3>

              <div className='relative w-15 h-15 sm:w-20 sm:h-20 mx-auto'>
                <CircularProgressbar
                  value = {percentage} 
                  text={`${score}/10`}
                  styles={buildStyles({
